...
impl SplitContract {
    ...
    fn _create_invoice_inner(
        ...
        // Issue #241: check creator self-imposed spending limit.
        let self_limit: i128 = env
            .storage()
            .persistent()
            .get(&creator_self_limit_key(&creator))
            .unwrap_or(0);
        if self_limit > 0 {
            ...
            let mut daily_used: i128 = if last_day == today_start {
                env.storage()
                    .persistent()
                    .get(&creator_self_used_key(&creator))
                    .unwrap_or(0)
            ...
            // The effective limit is the minimum of admin cap and self limit
            let effective_limit = if volume_cap > 0 && volume_cap < self_limit {
                volume_cap
            } else {
                self_limit
            };
            ...
            assert!(
                daily_used.checked_add(total).expect("self limit overflow") <= effective_limit,
                "creator self-imposed spending limit exceeded for today"
            );

            // Update the daily usage and current day marker
            env.storage()
                .persistent()
                .set(&creator_self_used_key(&creator), &(daily_used + total));
            env.storage()
                .persistent()
                .set(&creator_self_limit_day_key(&creator), &today_start);
        }
