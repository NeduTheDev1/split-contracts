    ...
    fn _release_full(env: &Env, invoice_id: u64, invoice: &mut Invoice, actor: &Address) {
        ...
        for i in 0..n {
            ...
            if let Some(ref out_token) = swap_token {
                ...
                // Issue #1: register payout as a streaming payment via the stream contract.
                let stream_ctr: Address = env
                    .storage()
                    .persistent()
                    .get(&stream_contract_key())
                    .expect("stream contract not set");
                // Transfer funds to the stream contract so it can release them over time.
                token_client.transfer(&env.current_contract_address(), &stream_ctr, &payout);
                // Register the stream: create_stream(recipient, amount, duration).
                // Duration defaults to 30 days.
                const THIRTY_DAYS: u64 = 30 * 24 * 60 * 60;
                let mut args: Vec<Val> = Vec::new(env);
                args.push_back(recipient.into_val(env));
                args.push_back(payout.into_val(env));
                args.push_back(THIRTY_DAYS.into_val(env));
                let _: Val = env.invoke_contract(
                    &stream_ctr,
